...
public class GalleryController {
   ...
   String gallery_list(Model model, GalleryDto dto)
   {
	  ...
	  model.addAttribute("totalCnt", galleryService.getTotal(dto));
...
//      String path = "C:\\Users\\itsok\\Desktop\\MLP full-stack\\Code\\MLP\\Spring\\workspace\\0124\\SpringDI_DB2\\upload";
//      //이 경로가 존재하는지 확인하기 
...
//      }
   ...
   String gallery_delete(GalleryDto dto) {
		galleryService.delete(dto);
		return "redirect:/gallery/list";
	}
   
   @RequestMapping("/gallery/modify")
   String gallery_modify(GalleryDto dto, Model model) {
	   GalleryDto resultDto = galleryService.getView(dto);
		model.addAttribute("galleryDto", resultDto);
		return "gallery/gallery_write";
	}
